...
def index():
    if request.method == 'POST':
        ...
        f.save(f.filename)
        scanned_file: List[str] = read_file(file_to_open=f.filename)
        if not scanned_file:
            logging.error(f"No sudoku found")
            sys.exit(1)
        initial_grid: List[List[int]] = create_sudoku(scanned_file)
        initial_grid_string = print_grid(description="Initial grid", grid=initial_grid)
        solutions, have_solution, information = solve_sudoku(grid=initial_grid)
        solved_grid_string: str = ''
        for i, solution in enumerate(solutions):
            solved_grid_string += print_grid(description=f"solution {i + 1}", grid=solution)
